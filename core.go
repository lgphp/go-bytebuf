...
func NewByteBufWithCapacity(capacity int32, buffer ...[]byte) (*ByteBuf, error) {
	if capacity < 1 {
		ChunkSize = int(1)
	} else {
		ChunkSize = int(capacity)
	}
	buf, e := NewByteBuf(buffer...)
	return buf, e
}

func NewDefaultByteBuf() (*ByteBuf, error) {
	buffer := make([]byte, 0)
	buf, e := NewByteBuf(buffer)
	return buf, e
}
...
func NewByteBuf(bytes ...[]byte) (*ByteBuf, error) {
	b := &ByteBuf{
...
// ForceRelease force release bytes
